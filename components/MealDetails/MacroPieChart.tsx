...
import { MealFoods } from "@/Types/types";

export function MacroPieChart({mealNameId}: { mealNameId: number}) {
  const { data, isLoading } = useQuery<MealFoods[]>({
    queryKey: ["MealDetails", mealNameId],
    queryFn: () => fetchGet<MealFoods[]>(`/api/Meals/${mealNameId}`),
  });
